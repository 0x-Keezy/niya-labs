...
export default function Chart({
  ...
  // Suppress the "Object is disposed" error that lightweight-charts emits
  // from its OWN internal ResizeObserver + paint loop after the chart is
  // torn down. The library's `fancy-canvas` dependency installs a
  // ResizeObserver we can't disconnect from the outside, so even though
  // `chart.remove()` is called in cleanup, one last paint can fire inside
  // a microtask and throw. Registering a global error listener that
  // silently swallows THIS SPECIFIC error keeps the Next.js dev overlay
  // from showing on /tools → back navigation. Production users never see
  // the overlay anyway; this is dev-quality-of-life.
  useEffect(() => {
    const onError = (event: ErrorEvent) => {
      const msg = event.error?.message ?? event.message ?? '';
      if (msg === 'Object is disposed' || msg.includes('Object is disposed')) {
