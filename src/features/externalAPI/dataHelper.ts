...
// GET Request Handlers - use defaults when config.json doesn't exist
export const handleGetConfig = () => readFile(configFilePath, defaults);
...
const updateConfig = (body: any) => {
  const currentConfig = readFile(configFilePath, defaults);
  if (body.key && body.value !== undefined) {
    const { key, value } = body;
    currentConfig[key] = value;
    writeFile(configFilePath, currentConfig);
