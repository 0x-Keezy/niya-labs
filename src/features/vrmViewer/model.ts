...
export class Model {
  ...
  private async fadeToAction(
    ...
    if (!destAction) {
      console.warn('[VRM Model] fadeToAction called with undefined destAction, skipping');
      return;
    }
    ...
    if (previousAction && previousAction !== this._currentAction) {
  ...
  public async playAnimation(
    ...
    // Store current idle action for restoration (may be undefined if no animation loaded yet)
    ...
    const restoreState = () => {
      ...
      // Only restore to idle if we had one before; fadeToAction handles undefined gracefully
      if (idleAction) {
