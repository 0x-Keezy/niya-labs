...
const avatarBones: VRMHumanBoneName[] = [
  "hips",
  "spine",
  "chest",
  "upperChest",
...
export class Viewer {
  ...
  // Mouse follow target for VRM lookAt
  private mouseFollowTarget: THREE.Object3D;
  private mouseFollowEnabled: boolean = true;
  ...
  constructor() {
    ...
    // Mouse follow target - positioned in front of camera
    this.mouseFollowTarget = new THREE.Object3D();
    this.mouseFollowTarget.name = "MouseFollowTarget";
  ...
  public async setup(canvas: HTMLCanvasElement) {
    ...
    try {
      ...
      console.warn("WebGL not available, using fallback mode:", e);
      this.isReady = false;
      return;
    }
    ...
    // Add mouse follow target to scene
    scene.add(this.mouseFollowTarget);

    // WebXR controller code removed - not needed for this project
    ...
    // Hide GUI panel completely - not needed for production
    ...
    // stats - reuse existing singleton if available, including panels
    let existingStats = document.getElementById('niya-stats-panel') as HTMLDivElement | null;
    let stats: Stats;
    
    if (existingStats && (existingStats as any).__statsInstance) {
      stats = (existingStats as any).__statsInstance;
      this.stats = stats;
      this.updateMsPanel = (existingStats as any).__updateMsPanel;
      this.renderMsPanel = (existingStats as any).__renderMsPanel;
      this.scenarioMsPanel = (existingStats as any).__scenarioMsPanel;
      this.physicsMsPanel = (existingStats as any).__physicsMsPanel;
      this.modelMsPanel = (existingStats as any).__modelMsPanel;
      this.bvhMsPanel = (existingStats as any).__bvhMsPanel;
      this.raycastMsPanel = (existingStats as any).__raycastMsPanel;
      this.statsMsPanel = (existingStats as any).__statsMsPanel;
    } else {
      if (existingStats) {
      ...
      stats = new Stats();
      this.stats = stats;
      stats.dom.id = 'niya-stats-panel';
      (stats.dom as any).__statsInstance = stats;
      ...
      this.scenarioMsPanel = stats.addPanel(new Stats.Panel("scenario_ms", "#f8f", "#221"));
      this.physicsMsPanel = stats.addPanel(new Stats.Panel("physics_ms", "#88f", "#212"));
      ...
      (stats.dom as any).__updateMsPanel = this.updateMsPanel;
      (stats.dom as any).__renderMsPanel = this.renderMsPanel;
      (stats.dom as any).__scenarioMsPanel = this.scenarioMsPanel;
      (stats.dom as any).__physicsMsPanel = this.physicsMsPanel;
      (stats.dom as any).__modelMsPanel = this.modelMsPanel;
      (stats.dom as any).__bvhMsPanel = this.bvhMsPanel;
      (stats.dom as any).__raycastMsPanel = this.raycastMsPanel;
      (stats.dom as any).__statsMsPanel = this.statsMsPanel;
    }
  ...
  public async loadVrm(
    ...
    // Connect mouse follow target to VRM lookAt for improved eye tracking
    this.setupMouseFollow();
  }

  /**
   * Setup mouse follow for VRM lookAt
   * Makes the avatar smoothly track the cursor position
   */
  private setupMouseFollow(): void {
    if (!this.model?.vrm?.lookAt) {
      console.log("VRM lookAt not available for mouse follow");
      return;
    }

    // VRMLookAtSmoother has userTarget property for user direction tracking
    const lookAt = this.model.vrm.lookAt as any;
    
    // Ensure autoUpdate is enabled for lookAt to work
    lookAt.autoUpdate = true;
    
    // Set the target - required for lookAt.update() to process
    lookAt.target = this.mouseFollowTarget;
    
    // Set the userTarget - this works with VRMLookAtSmoother for user direction
    lookAt.userTarget = this.mouseFollowTarget;
    
    console.log("Mouse follow enabled for VRM lookAt:", {
      hasTarget: !!lookAt.target,
      hasUserTarget: !!lookAt.userTarget,
      autoUpdate: lookAt.autoUpdate
    });
  }

  /**
   * Update mouse follow target position based on cursor
   * Called every frame in the update loop
   */
  private updateMouseFollow(): void {
    if (!this.mouseFollowEnabled || !this.camera) return;

    // Get the VRM head bone position for accurate targeting
    const headBone = this.model?.vrm?.humanoid?.getRawBoneNode("head");
    if (!headBone) return;
    ...
    const distanceToHead = cameraPos.distanceTo(headWorldPos);
    
    // Create a target point on a plane at the head's depth
    // mouse.x/y range from -1 to 1
    // Scale multipliers based on distance for consistent behavior across zoom levels
    const scaleFactor = Math.max(0.5, Math.min(2.0, distanceToHead / 2.0));
    
    // Asymmetric Y: softer upward (0.25) to avoid snap, normal downward (0.35)
    const yMultiplier = this.mouse.y > 0 ? 0.25 : 0.35;
    
    // Create offset in camera space, then transform to world
    const offset = new THREE.Vector3(
      this.mouse.x * 0.35 * scaleFactor,  // Horizontal tracking
      this.mouse.y * yMultiplier * scaleFactor,  // Vertical tracking
      0  // No depth offset - we'll position at head depth
    );
    
    // Get camera's right and up vectors for proper offset calculation
    const cameraRight = new THREE.Vector3();
    const cameraUp = new THREE.Vector3();
    this.camera.matrixWorld.extractBasis(cameraRight, cameraUp, new THREE.Vector3());
    
    // Calculate target position: start from head, offset by mouse in camera space
    const targetPos = headWorldPos.clone()
      .add(cameraRight.multiplyScalar(offset.x))
      .add(cameraUp.multiplyScalar(offset.y));
    
    // Smooth interpolation for gentle transitions
    this.mouseFollowTarget.position.lerp(targetPos, 0.10);
  }

  /**
   * Enable or disable mouse follow
   */
  public setMouseFollowEnabled(enabled: boolean): void {
  ...
  public createBallAtPoint(point: THREE.Vector3, itype: number = 0) {
    ...
    const color = itype == 0 ? getAvatarColor() : getRoomColor();
  ...
  public updateRaycasts() {
    const checkIntersection = (closestPart: THREE.Object3D) => {
      ...
      const handleAvatarIntersection = (point: THREE.Vector3) => {
      ...
      if (this.intersectsModel.length > 0 && this.intersectsRoom.length > 0) {
        if (
          ...
          handleAvatarIntersection(this.intersectsModel[0].point);
        } else {
  ...
  public async loadScenario(url: string) {
    ...
    // SECURITY: scenarios are executed via `new Function()` — they are
    // effectively arbitrary JavaScript running in the user's session.
    // Restrict the URL to same-origin (plus an optional allowlist base via
    // NEXT_PUBLIC_SCENARIO_BASE_URL) to prevent RCE if ever fed an external
    // URL. Refuse to run server-side; scenarios are a browser-only feature.
    if (typeof window === "undefined") {
      throw new Error("[viewer] loadScenario cannot run on the server");
    }
    const allowedBases: string[] = [window.location.origin];
    const extra = process.env.NEXT_PUBLIC_SCENARIO_BASE_URL;
    if (extra) allowedBases.push(extra.replace(/\/$/, ""));
    const originOk = allowedBases.some(
      (base) => url === base || url.startsWith(base + "/"),
    );
    if (!originOk) {
      throw new Error(
        `[viewer] loadScenario rejected: URL "${url}" is not in the allowlist ` +
          `(${allowedBases.join(", ")}). Scenarios execute arbitrary code and ` +
  ...
  public update(time?: DOMHighResTimeStamp, frame?: XRFrame) {
    ...
    // Update mouse follow target position for VRM lookAt
