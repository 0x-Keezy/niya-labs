...
import { ExpressionPreviewPage } from "./settings/ExpressionPreviewPage";
...
export const Settings = ({
  ...
  const [isAdmin, setIsAdmin] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('niya_admin_auth') === 'true';
    }
    return false;
  });
  ...
  async function handleAdminLogin() {
    try {
      ...
      if (data.success) {
        setIsAdmin(true);
        localStorage.setItem('niya_admin_auth', 'true');
        setAdminError('');
        setAdminPassword('');
      } else {
  ...
  function handleAdminLogout() {
    setIsAdmin(false);
    localStorage.removeItem('niya_admin_auth');
    setPage('main_menu');
    setBreadcrumbs([]);
  }

  const publicMenuKeys = ["language", "community"];
  const adminMenuKeys = ["appearance", "chatbot", "language", "tts", "stt", "vision", "expression_preview", "developer", "external_api", "reset_settings", "community"];
  const adminOnlyPages = new Set([
    "appearance", "chatbot", "tts", "stt", "vision", "expression_preview", "developer", "external_api", "reset_settings",
    "background_img", "background_color", "background_video", "character_model", "character_animation",
    "chatbot_backend", "name", "system_prompt", "arbius_llm_settings", "chatgpt_settings", "llamacpp_settings",
    "ollama_settings", "koboldai_settings", "moshi_settings", "openrouter_settings",
    "tts_backend", "elevenlabs_settings", "speecht5_settings", "coquiLocal_settings", "openai_tts_settings",
    "piper_settings", "localXTTS_settings", "kokoro_settings", "rvc_settings",
    "stt_backend", "stt_wake_word", "whisper_openai_settings", "whispercpp_settings",
    "vision_backend", "vision_llamacpp_settings", "vision_ollama_settings", "vision_openai_settings", "vision_system_prompt"
  ]);
  ...
  function renderPage() {
    ...
    case 'main_menu':
      return (
        <div className="space-y-4">
          <MenuPage
            keys={isAdmin ? adminMenuKeys : publicMenuKeys}
            menuClick={handleMenuClick} />
          
          {!isAdmin ? (
            <div className="rounded-xl shadow-lg bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 border border-amber-500/20">
              <h3 className="text-sm font-semibold text-amber-400 mb-3">Admin Access</h3>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg bg-slate-700/50 border-amber-500/30 text-slate-200 placeholder-slate-500 shadow-sm text-sm p-2 border focus:border-amber-500 focus:ring-amber-500/20 focus:ring-2 focus:outline-none"
                  data-testid="input-admin-password"
                />
                <button
                  onClick={handleAdminLogin}
                  className="px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-lg text-sm font-medium hover:from-amber-400 hover:to-amber-500 transition-all shadow-lg shadow-amber-500/20"
            ...
            <div className="rounded-xl shadow-lg bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 border border-amber-500/20">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm text-white bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 rounded-lg font-medium transition-all"
    ...
    case 'expression_preview':
      return <ExpressionPreviewPage />;
  ...
  return (
    <div
      className="absolute inset-0 text-slate-200 text-xs text-left z-30 backdrop-blur-md flex flex-col"
    >
      <div
        className="absolute inset-0 bg-slate-900/90 pointer-events-none"
      ></div>
      <div className="relative flex-1 overflow-y-auto overscroll-contain" style={{ scrollbarGutter: 'stable' }}>
      <div
        className="sticky w-full top-0 left-0 z-50 p-2 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border-b border-amber-500/30 shadow-lg shadow-amber-500/5"
        ref={topMenuRef}
      >
        <div className="flex items-center justify-between max-w-4xl mx-auto">
          <div className="flex items-center gap-3">
            <span className="text-amber-500 font-bold text-lg tracking-wider">NIYA</span>
            <span className="text-slate-500 text-sm">Settings</span>
          </div>
            ...
            <ol role="list" className="flex items-center space-x-4">
              <li className="flex">
                <div className="flex items-center">
                  <span
                    ...
                    className="text-amber-500/70 hover:text-amber-400 cursor-pointer transition-colors"
                  >
              ...
              {breadcrumbs.map((breadcrumb) => (
                <li key={breadcrumb.key} className="flex">
                  <div className="flex items-center">
                    <ChevronRightIcon className="h-5 w-5 flex-shrink-0 text-slate-600" aria-hidden="true" />
                    <span
                      onClick={() => {
                      ...
                      className="ml-4 text-sm font-medium text-slate-400 hover:text-amber-400 cursor-pointer transition-colors"
                    >
                      {breadcrumb.label}
                    </span>
                  </div>
      ...
      <div className="min-h-full opacity-95">
      ...
      <div
        aria-live="assertive"
        className="pointer-events-none absolute inset-0 flex items-end px-4 py-6 sm:items-start sm:p-6 mt-2 z-50"
