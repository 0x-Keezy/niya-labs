...
export function basicPage(
  ...
  return (
    <>
      <div className="rounded-xl shadow-lg bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-5 border border-amber-500/20">
        <h2 className="text-xl w-full text-amber-400 font-semibold">{title}</h2>
        <p className="w-full my-4 text-slate-300">{description}</p>
...
export function getIconFromPage(page: string): JSX.Element {
  switch(page) {
    ...
    case 'niya_life':          return <SunIcon className="h-5 w-5 flex-none text-gray-800" aria-hidden="true" />;
    ...
    case 'expression_preview':  return <FaceSmileIcon className="h-5 w-5 flex-none text-purple-500" aria-hidden="true" />;
...
function getLabelFromPage(page: string): string {
  ...
  switch(page) {
    ...
    case 'niya_life':          return t('Niya Life');
    ...
    case 'expression_preview':  return t('Expression Preview');
